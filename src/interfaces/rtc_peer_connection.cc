...
static Validation<Napi::Value> CreateRTCPeerConnectionIceErrorEvent(
    const Napi::Value hostCandidate, const Napi::Value url,
    const Napi::Value errorCode, const Napi::Value errorText) {
  auto env = hostCandidate.Env();
  Napi::EscapableHandleScope scope(env);
  NODE_WEBRTC_CREATE_OBJECT_OR_RETURN(env, object)
  NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(env, object, "hostCandidate",
                                        hostCandidate)
...
void RTCPeerConnection::OnIceCandidateError(const std::string &host_candidate,
